...
type FormErrors = Partial<Record<keyof DiplomadoRequest, boolean>>;

const DiplomadoFormPage = () => {
    const [form, setForm] = useState<DiplomadoRequest>({
        nombre: "",
        objetivos: "",
        semestres: 0,
        publico_objetivo: "",
    ...
    useEffect(() => {
        if (slug) fetchDiplomado()
    }, [slug]);

    const fetchDiplomado = () => {
        if (!slug) return
        DiplomadoService.get(slug).then((curso) => {
            setImage(curso.imagen_url ?? "");
            setForm({
                area: curso.area,
                nombre: curso.nombre,
                objetivos: curso.objetivos,
                publico_objetivo: curso.diplomado?.publico_objetivo,
    ...
    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        if (formErrors[name as keyof DiplomadoRequest]) {
    ...
    const validateForm = (form: DiplomadoRequest): FormErrors => {
    ...
    const createDiplomado = () => {
        DiplomadoService.create(form).then((curso) => {
    ...
    const updateDiplomado = () => {
        if (!slug) return
        DiplomadoService.update(slug, form).then((curso) => {
            saveImage(curso.slug);
        }).catch(error => {
            setIsSending(false);
            setRequestError(error.message)
    ...
    const saveImage = (cursoSlug: string) => {
        if (!imageFile){
            navigate({ pathname: Routes.ADMIN.CURSO.LISTAR, search: "?tipo=diplomados" });
            return;
        }
...
export default DiplomadoFormPage;